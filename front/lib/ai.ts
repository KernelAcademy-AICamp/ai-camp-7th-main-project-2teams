...
5. 어느 대분류도 안 맞거나 툴·로그인·정체불명이면 태그없음(빈 배열)(억지 태깅 금지). 단 도메인·서비스가 명백하면(아래 중분류 주의) 로그인/앱 화면이어도 분류 — title이 URL 통짜·회사명 단독이어도 도메인 지식으로 분류 시도, 정말 불명확할 때만 태그없음(빈 배열).

## 중분류 주의
- RAG는 "검색증강생성 기법 자체"를 다룰 때만. LLM API·모델 카드·일반 문서는 중분류 "LLM".
- LLM 챗 서비스(ChatGPT·Claude·Gemini 등) 페이지는 로그인/앱 화면이어도 대분류 "AI/ML", 중분류 "LLM". (정체불명 툴·일반 대시보드는 여전히 태그없음(빈 배열) — 여기 명명된 AI 챗 서비스에 한정.)
- AI 코딩 에이전트/툴(Claude Code·Codex·Cursor·Windsurf 등) 자체의 사용법·설정·비교·내부동작이 주제면 대분류 "AI/ML"(중분류는 강의 콘텐츠면 "온라인 강의" 규칙 우선 적용해 "강의", 아니면 "LLM"). 그 툴을 도구로 써서 다른 도메인 산출물(프론트엔드 앱·자동화 등)을 만드는 과정 자체가 주제면 그 산출물의 도메인(예: n8n으로 GitHub 자동화 만들기="개발").
...
- 유튜브 등 플랫폼: 플랫폼명 태그 금지, 영상 주제로 분류. 커리큘럼형 강의 시리즈=학습>강의>주제, 단발 주제 영상(튜토리얼·공략·레시피 등)=주제 대분류. 음악·브이로그·예능 등 매핑 대분류 없으면 태그없음(빈 배열).
- 유튜브 채널·크리에이터 페이지(/@handle 등): 채널이 다루는 분야 대분류로 분류. 중분류는 채널이 단일 분야에 명확히 특화됐을 때만 추가(프론트엔드 전문 채널→개발>프론트엔드). 풀스택·범용·다분야 채널은 대분류만(웹·앱 풀스택/여러 언어·분야 개발 강의 채널→개발). 교육 채널도 학습>강의 아닌 분야 대분류로. 가수·브이로거 등 매핑 대분류 없으면 태그없음(빈 배열).
...
- 여러 언어·프레임워크를 동시에 다루는 저장소(코딩 규칙/컨벤션 모음, 멀티 프레임워크 스타터·보일러플레이트, 크로스플랫폼 설정 모음 등)는 그중 하나만 소분류로 골라 쓰지 않음 — 제목·설명에 특정 기술이 대표로 명시되지 않는 한 소분류 생략(대분류 "개발" + 중분류 "오픈소스" 정도로 그침).
...
- 라이프스타일: 운동·요리·인테리어·육아·건강(웰니스)·자동차 일상 콘텐츠. 직접 조리 레시피=요리, 차량 정보·관리=자동차. 증상·약·진단 등 임상 의료는 대분류 부여 말 것(확신 없으면 태그없음(빈 배열)).
...
제목: How I use Claude Code (Senior Software Engineer Tips) → {"tags":[{"tag":"AI/ML","confidence":0.9},{"tag":"LLM","confidence":0.8}]}
제목: n8n으로 끝내는 AI GitHub 에이전트 (1일 1커밋 자동화) → {"tags":[{"tag":"개발","confidence":0.9},{"tag":"자동화","confidence":0.8},{"tag":"n8n","confidence":0.8}]}
...
제목: rules / URL: github.com/example/agent-rules (TS·Python·Go·Swift 등 여러 언어 코딩 규칙 모음, 설명에 특정 프레임워크 언급 없음) → {"tags":[{"tag":"개발","confidence":0.9},{"tag":"오픈소스","confidence":0.8}]}
