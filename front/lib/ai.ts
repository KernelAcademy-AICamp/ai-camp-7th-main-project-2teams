...
중분류: 대분류 하위 분야 (예: 개발→프론트엔드, AI/ML→RAG, 커뮤니티→포럼, 콘텐츠→블로그, 브랜드→마케팅, 게임→공략, 라이프스타일→요리, 여행→숙소, 금융→부동산)
...
- 공공데이터·통계·행정 포털(공공데이터포털·KOSIS·통계청 등)은 대분류 "비즈니스", 중분류 "데이터". 대분류 신설 안 함.
- 디자인 스튜디오·에이전시 회사 홈(포트폴리오·작업 소개)은 대분류 "디자인", 중분류 "스튜디오"(브랜드 아님). 브랜드는 소비재 제품 브랜드(Nike 등)에 한정.
...
제목: pxd / URL: pxd.co.kr (UX 디자인 스튜디오) → {"tags":[{"tag":"디자인","confidence":0.85},{"tag":"스튜디오","confidence":0.8}]}
