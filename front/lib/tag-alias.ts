...
// 대소문자 변형 fallback용 소문자 사전. 정확 일치가 항상 우선 —
// 'UI'(대분류 디자인) vs 'ui'(중분류 UI/UX)처럼 케이스로 구분되는 기존 매핑을 깨지 않는다.
const TAG_ALIAS_LOWER = new Map(
  Object.entries(TAG_ALIAS).map(([k, v]) => [k.toLowerCase(), v]),
)
const CATEGORY_ALIAS_LOWER = new Map(
  Object.entries(CATEGORY_ALIAS).map(([k, v]) => [k.toLowerCase(), v]),
)

// 정확 일치 → 소문자 fallback 순. 'Frontend'/'FRONTEND' 같은 케이스 변형이
// alias를 놓쳐 같은 개념 태그가 표기 분열되는 문제 방지.
function resolveAlias(tag: string): string {
  return (
    TAG_ALIAS[tag] ??
    CATEGORY_ALIAS[tag] ??
    TAG_ALIAS_LOWER.get(tag.toLowerCase()) ??
    CATEGORY_ALIAS_LOWER.get(tag.toLowerCase()) ??
    tag
  )
}

// 서로 다른 원본 태그가 같은 alias로 매핑될 수 있어(예: infra/DevOps/배포 → 인프라) dedupe 필수.
// trim 선행 — 공백 포함 태그(" 개발 ")가 alias·TOP_CATEGORIES 정확일치를 빠져나가는 것 방지.
...
export function resolveTopCategory(input: string): string | null {
  ...
  const resolved =
    CATEGORY_ALIAS[trimmed] ?? CATEGORY_ALIAS_LOWER.get(trimmed.toLowerCase()) ?? trimmed
