...
// 재실측(2026-07-28, n=215 — 제휴마케팅 표본 3건 추가 + 쇼핑 경계규칙 도입):
//   경계규칙 前  rich F1 0.7741 · 대분류 0.8605 · exact 0.4651 / title-only F1 0.7478 · 대분류 0.8233
//   경계규칙 後  rich F1 0.7893 · 대분류 0.8651 · exact 0.4977 / title-only F1 0.7664 · 대분류 0.8326
//   → 두 패스 전 지표 상승(rich +0.015 / title-only +0.019, exact는 +0.033·+0.042로 더 큼).
//     오분류 3건 교정 + 정상 쇼핑 항목 과교정 0건. baseline 값은 유지 — 마진만 넓어졌다.
//     (n=213→215 자체는 F1을 낮추는 방향이었다. 추가 3건이 도입 전엔 전부 오답)
// 재실측(2026-07-28, n=215 — 로그인·인증 화면 경계 규칙 도입 후. **현행 최신값**):
//   rich       대분류 0.8651 → 0.8744 · emptyRate 0.030 → 0.015 (rich F1은 미기록 — 직전 실측 0.7893)
//   title-only F1 0.7664 → 0.7809 · 대분류 0.8326 → 0.8512 · emptyRate 0.070 → 0.035
//   → 판단 기준을 경로가 아니라 **서비스 식별 가능 여부**로 통일. github.com/login=`개발>도구>GitHub`,
//     accounts.google.com/signin=[](특정 제품이 아닌 계정 인증 진입점). SYSTEM_PROMPT 내부 모순
//     (0태그 조건이 "로그인 페이지"를 무조건 열거 vs 대분류 규칙은 "도메인 명백하면 분류") 해소.
//   ⚠ **예제를 추가하지 말 것** — figma.com/login few-shot 추가안은 `도구` 중분류 가중이 커져
//     `큐레이션`을 잠식(behance·heroicons), rich F1 0.7726으로 하락해 기각. 채택안은 기존 ChatGPT
//     예제의 URL만 로그인 경로로 교체해 예제 수를 유지한 것이다(../ai.ts:67).
//   상세: docs/specs/tag-taxonomy.md §경계 규칙(로그인·인증 화면, 2026-07-28 확정)
