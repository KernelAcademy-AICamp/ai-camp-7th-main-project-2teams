...
- [ ] A58: 자연어 검색 + 태그/즐겨찾기 필터 조합 (high)
- [ ] A59: 중복 북마크 경고 UI (high)
- [ ] A60: 태그/카테고리 사용자 직접 편집 + 카드 수정 (high)
- [ ] A61: 파일 임포트 고도화 (medium)
- [ ] A62: 대량 북마크 페이지네이션 개선 (medium)
- [ ] A63: 카카오 소셜 로그인 (medium)
...
| --- | ------------------------------------------ | -------- | ------- | ------------------------------------------------------------- |
| A58 | 검색 + 태그/즐겨찾기 필터 조합            | high     | pending | A55로 category 필터는 완료. tags/is_favorite만 추가.          |
| A59 | 중복 북마크 경고 UI                       | high     | pending | 백엔드 409(A35)는 완료. 토스트 UI만 남음.                      |
| A60 | 태그·카테고리 편집 + 카드 수정            | high     | pending | description 컬럼(0013 마이그레이션)만 추가된 상태. 편집 UI 없음. |
| A61 | 파일 임포트 고도화                        | medium   | pending | 속도표시/결과요약/로그/부분완료/에러상세/문제보고 전부 미착수. |
| A62 | 대량 북마크 페이지네이션 개선             | medium   | pending | 현재 전체 조회 방식, cursor 페이지네이션 미도입.               |
| A63 | 카카오 소셜 로그인                         | medium   | pending | Supabase 기본 미지원 프로바이더 — 커스텀 OIDC 연동 필요. 네이버 스코프 제외. |
