...
> 실측 기준일: 2026-07-13 (태깅 gpt-4o-mini, 검색 text-embedding-3-small).
...
모델 text-embedding-3-small. 골든셋 북마크 20건 삽입 후 질의 14건 채점, finally에서 정리.
...
| 게이트 | 기준 | 실측 | 판정 |
|---|---|---|---|
| overall recall | ≥ 0.75 | 0.917 (n=12) / floor 0.786 (n=14) | ✅ |
| non-weak-vector recall (진짜 품질 게이트) | ≥ 0.90 | **1.0** | ✅ |
| MRR / hitRate | 참고 | 0.917 (n=12) | — |

> weak-vector는 title-only 임베딩의 **구조적 한계**(회귀 아님) — baseline은 이 실패를
> 전제로 잡되, 그 외 카테고리 하락은 잡도록 non-weak 게이트를 별도 유지.
> N-2에서 weak-vector 표본을 1→3으로 확대(분모 증가)하며 overall baseline 0.83→0.75 재보정.
...
| 검색 | non-weak recall ≥ 0.90 | 1.0 | 🟢 통과 |
| 검색 | overall recall ≥ 0.75 | 0.79~0.92 | 🟢 통과 |
