...
| 게이트 | 기준 | 실측 | 판정 |
|---|---|---|---|
| rich macro-F1 | ≥ **0.74** | **0.755~0.768** (3회 측정 밴드, 최신 0.7595) | ✅ |
| title-only macro-F1 (floor) | ≥ 0.73 | **0.748** (07-25 재측정) | ✅ |
| emptyRate (미분류율 상한) | ≤ 0.15 | rich 0.030 · title 0.066 | ✅ |
| 대분류 정확도 (참고) | — | 0.845~0.859 | — |
...
|---|---|---|---|
| **환각** | 근거 없는 태그 생성 | precision 하락 | 0.754 |
| **누락** | 있어야 할 태그 미생성 | recall · emptyRate | 0.781 / 0.030 |
| **편향** | 특정 대분류 쏠림·과소 | 대분류 정확도 | 0.854 |
| **형식** | 표기 분열(파이썬↔Python) | exact-match · alias 정규화 | 0.446 |
...
| 평가 | 핵심 게이트 | 실측 | 상태 |
|---|---|---|---|
| 태깅 | macro-F1 ≥ 0.74 | 0.755~0.768 (분산 밴드) | 🟢 통과 |
...
- 태깅 소분류 정밀도(exact 0.446) — alias 사전 지속 보강
