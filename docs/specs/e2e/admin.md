...
18. `/api/admin/stats`가 500을 반환하는 환경(예: RPC 오류 재현)에서 관리자 계정으로 `/admin` 접속
19. "대시보드 데이터를 불러오지 못했습니다" 에러 메시지 노출 확인 — OKR 타일·카테고리 도넛이 깨진 값으로 렌더되거나 페이지가 크래시하지 않음
20. OpenAI 사용량 위젯은 별도 API이므로 stats 실패와 무관하게 정상 동작(또는 저니 D대로 조회 불가) 확인
