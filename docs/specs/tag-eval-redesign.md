...
**C-2. 정책 결정 필요 항목 (프롬프트 반영 전제)**
1. LLM 챗 툴(ChatGPT·Claude·Gemini)을 AI/ML로 분류할 것인가, 0태그(툴/로그인)로 둘 것인가?
2. 공공데이터/통계/행정 서비스의 대분류 — 현 13개에 없음. 신설 vs 기존 매핑?
3. 디자인 스튜디오 회사 페이지 — 브랜드 vs 디자인 우선순위?

### 시나리오 D — 회귀 게이트 강화 (우선순위 2)

**D-1. 미분류율 지표 추가**
- `tag-eval.ts` `aggregate()`에 `emptyRate`(gold 있는데 예측 빈 비율) 추가.
- 게이트: baseline F1 유지 **AND** emptyRate 상한(예: 0.15) 이하.

**D-2. 대분류 정확도 + 미분류 동시 감시**
- F1만으로는 "태그 삭제로 오답 회피"가 통과됨. emptyRate 병행으로 차단.

## 3. 실행 순서 (권장 — §0 판정 반영)

1. **B-1, B-2** 즉시 — retag 스크립트 안전장치. 다음 retag부터 순손실 방지.
2. **C-1, C-2** — 골든셋에 저품질 title 패턴 편입 + 정책 결정. **미분류 감축 실질 지렛대(주원인 대응).**
3. **A-1** — 평가 이중 모드. retag 실제 품질 가시화.
4. **D-1, D-2** — 게이트 강화(F1+미분류율). 회귀 자동 차단.
5. **A-2** — 입력 보강(description). **효과 제한적(미분류 -2%p, §0)** — title 정제/URL·로그인 폴백을 먼저 검토 후 보완재로.
...
- [ ] LLM 챗 툴 분류 여부 (§C-2.1)
- [ ] 공공데이터/통계 대분류 신설 여부 (§C-2.2)
- [ ] 디자인 스튜디오 브랜드/디자인 우선순위 (§C-2.3)
