...
같은 파일 안에 동일 URL이 여러 번 들어있는 경우(브라우저 북마크 업로드에서 흔함)도 각각 별도로 AI 처리된다.
...
```ts
function dedupeBatch(items: ParsedBookmark[]): {
  candidates: Map<string, ParsedBookmark & { url: string }>;
  duplicate: number;
} {
